 ...
 * @since      Moodle 3.0
 */
class mod_embeddedvideopage_external extends external_api {
     ...
     * @since Moodle 3.0
     */
    public static function view_embeddedvideopage_parameters() {
    ...
    public static function view_page($pageid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/embeddedvideopage/lib.php");

        $params = self::validate_parameters(self::view_embeddedvideopage_parameters(),
                                            array(
                                                'pageid' => $pageid
                                            ));
        $warnings = array();
        ...
        $page = $DB->get_record('embeddedvideopage', array('id' => $params['pageid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($page, 'embeddedvideopage');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        require_capability('mod/embeddedvideopage:view', $context);

        // Call the page/lib API.
        embeddedvideopage_view($page, $course, $cm, $context);
     ...
     * @since Moodle 3.0
     */
    public static function view_embeddedvideopage_returns() {
    ...
    public static function get_pages_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $pages = get_all_instances_in_courses("embeddedvideopage", $courses);
            foreach ($pages as $page) {
                ...
                list($page->intro, $page->introformat) = external_format_text($page->intro,
                                                                $page->introformat, $context->id, 'mod_embeddedvideopage', 'intro', null);
                $page->introfiles = external_util::get_area_files($context->id, 'mod_embeddedvideopage', 'intro', false, false);

                $options = array('noclean' => true);
                list($page->content, $page->contentformat) = external_format_text($page->content, $page->contentformat,
                                                                $context->id, 'mod_embeddedvideopage', 'content', $page->revision, $options);
                $page->contentfiles = external_util::get_area_files($context->id, 'mod_embeddedvideopage', 'content');

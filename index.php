...
$event = \mod_embeddedvideopage\event\course_module_instance_list_viewed::create(array('context' => context_course::instance($course->id)));
$event->add_record_snapshot('course', $course);
$event->trigger();

$strpage         = get_string('modulename', 'embeddedvideopage');
$strpages        = get_string('modulenameplural', 'embeddedvideopage');
$strname         = get_string('name');
$strintro        = get_string('moduleintro');
$strlastmodified = get_string('lastmodified');

$PAGE->set_url('/mod/embeddedvideopage/index.php', array('id' => $course->id));
$PAGE->set_title($course->shortname.': '.$strpages);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($strpages);
echo $OUTPUT->header();
echo $OUTPUT->heading($strpages);
if (!$pages = get_all_instances_in_course('embeddedvideopage', $course)) {
...
foreach ($pages as $page) {
    ...
    $table->data[] = array (
        ...
        format_module_intro('embeddedvideopage', $page, $cm->id));
}
